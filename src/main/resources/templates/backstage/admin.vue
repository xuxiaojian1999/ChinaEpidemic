            ...
            <div class="row clearfix">
              <div class="col-md-12 column">
                <form role="form">
                  ...
                  <div class="form-group">
					<pre>The one is the system administrator,the two is the staffl!</pre>
  ...
  methods:{
    ...
    saveModal(){
      ...
      //检验
        ...
        !pattern.test(this.userItem.account)||!pattern.test(this.userItem.phone)){
    ...
    getAllUserList(){
      ...
      .then(response => {
        //从数据库中获取到全部user
         ...
         if(result!=null&&result!='')this.allUserList= result
        //wathc监听alluserlist,自动重新设置userlist
        })
    ...
    deleteUserFromdb(id){
      ...
      .then(response => {
         ...
         if(result!=null&&result!=''){
            if(result)this.deleteUser(this.userItem.id)
         }
      })
      .catch(function (error) { // 请求失败处理
        console.log(error);
    ...
    modifyUserFromdb(){
                //异步请求
                this.$axios.post('/user/modifyUser', {
